...
public interface PersonMapper {
    ...
    @Select("SELECT first_name, last_name FROM people WHERE first_name = #{firstName} ORDER BY person_id ASC LIMIT #{_pagesize} OFFSET #{_skiprows}")
    Person selectPage(@Param("firstName") String firstName, @Param("_skiprows") int skipRows, @Param("_pagesize") int pageSize);

    @ResultMap("person")
    @Select("SELECT first_name, last_name FROM people ORDER BY person_id ASC LIMIT #{_pagesize} OFFSET #{_skiprows}")
